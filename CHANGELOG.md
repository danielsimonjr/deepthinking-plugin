...
Docs and planning work — no source code changes.

### Added

- **`docs/ROADMAP-FUTURE-MODES-AND-FORMATS.md`** — forward-looking inventory of reasoning mode and output format candidates. 0 Tier 1 modes (the plugin's 34 canonical modes are feature-complete); 1 Tier 2 mode (`decisionanalysis`, held pending a design spike); 2 Tier 1 formats (`latex-math`, `csv`) scheduled for v0.5.0; 3 Tier 2 formats and an extensive Tier 3 with rejection rationale. Includes a "Strategic gaps" section covering mode-count ceiling, smoke test scalability, mode deprecation path, telemetry gap, and hybrid mode evolution.
- **`docs/SKILL-INVARIANTS.md`** — catalog of per-mode validation rules that JSON Schema cannot express: 4 schema-unenforceable cardinality rules (`counterfactual`, `modal`, `historical`, `abductive`), 3 already enforced in schemas (`gametheory`, `synthesis`, `engineering`), and 2 cross-field semantic rules (`firstprinciples` confidence chain, `bayesian` arithmetic display). When editing a SKILL.md file, preserve any "must" / "exactly one" / "at least N" language.
- **`docs/PLAN-v0.5.0.md`** — agent-driven implementation plan for the next release. 16 tasks organized in 3 dependency-ordered waves (5 parallel Wave 1 tasks, 6 parallel Wave 2 cascade tasks, 5 sequential Wave 3 release-gate tasks). Each task includes a self-contained subagent brief with current state, goal, implementation guidance, acceptance criteria, and commit message. v0.5.0 scope: 2 new format grammars (`latex-math`, `csv`), 3 quality-infrastructure items (`test_version_consistency.py`, `MODE_DISPLAY_NAMES` sync check, smoke test parallelization), and 1 ergonomic fix (`think-render` alias). No new reasoning modes.

### Changed

- **`CLAUDE.md`** — expanded with:
  - **34-mode invariant promoted from 9 places to 10 places.** Added `MODE_DISPLAY_NAMES` dict in `scripts/render-html-dashboard.py` as the 10th sync location (find with `grep -n MODE_DISPLAY_NAMES scripts/render-html-dashboard.py`). No automated check enforces this — drift surfaces only as a wrong dashboard title (the fallback `mode.capitalize() + " Reasoning"` is wrong for compound names like `gametheory` or `firstprinciples`).
  - **New design principle: "Some validation rules live in SKILL.md prose, not in JSON Schema."** Full per-mode catalog in `docs/SKILL-INVARIANTS.md`.
  - **New "Out of scope" section** documenting deliberate non-goals: no persistent session state across `/think` invocations, no Node.js runtime, no hard `dot`/`mmdc` dependency, no third-party test framework.
  - **New "Skills vs slash commands" architectural section.** Skills (`skills/<category>/SKILL.md`) are auto-invoked by Claude based on conversation context; slash commands (`commands/<name>.md`) are user-typed. To add a user-facing `/foo` command, create `commands/foo.md`, NOT `skills/foo/SKILL.md`.
  - **New "Release sequence (don't reorder)"** under Publishing — documents the 7-step release gate with a warning that hardcoded equality assertions in tests are a smell.
